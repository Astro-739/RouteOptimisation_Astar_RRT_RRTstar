...
from RRT_utils import TreeNode
from RRT_utils import TreePath
...
class RRTAlgorithm:          
    ...
    def nodeCost_RRTstar(self,node:TreeNode,parentnode:TreeNode) -> None:
        # set node cost
        node.node_cost = RRT_utils.nodeDistance(node,parentnode)
        # set path cost to this node
    ...
    def pathCostLOS(self,goalnode:TreeNode) -> None:
        ...
        while LOSnode.location is not self.startnode.location:
            LOSnode.nodeLOS_cost = int(RRT_utils.nodeDistance(LOSnode,LOSnode.pathLOS_parent))
    ...
    def neighbourhoodNodes(self,node:TreeNode) -> list:
        ...
        for treenode in self.randomtree.nodes:
            distance = RRT_utils.nodeDistance(treenode,node)
    ...
    def lowestCostNeighbour(self,node:TreeNode,nearestnode:TreeNode) -> TreeNode:
        ...
        for neighbournode in node.neighbourhood:
            ...
            newpathcost = neighbournode.path_cost + RRT_utils.nodeDistance(node,neighbournode)
    ...
    def rewireTree(self,node:TreeNode) -> None:
        ...
        for neighbournode in node.neighbourhood:
            # -
            newpathcost = node.path_cost + RRT_utils.nodeDistance(node,neighbournode)
    ...
    def generateStep(self,nearestnode:TreeNode,targetnode:TreeNode) -> bool:                     
        dist = RRT_utils.nodeDistance(nearestnode,targetnode)
        ...
        if RRT_utils.nodeDistance(stepnode,self.goalnode) <= (self.STEPSIZE + self.GOALRADIUS):
        ...
        if not RRT_utils.isFreeSpaceCircle(stepnode,self.obstacles,self.SAFETYMARGIN):
            return False
        ...
        if RRT_utils.crossCircleObstacle(parentnode,stepnode,self.obstacles,self.SAFETYMARGIN):
            return False
    ...
    def randomStep(self) -> None:
        ...
        while (not free_randompoint) and while_loops < 20:
            randomnode = TreeNode(RRT_utils.drawRandomSample(self.mapwidth,self.mapheight))  
            free_randompoint = RRT_utils.isFreeSpaceCircle(randomnode,self.obstacles,self.SAFETYMARGIN)        
            while_loops += 1
        ...
        nearestnode = RRT_utils.nearestNeighbour(randomnode,self.startnode,self.randomtree,self.RRTstar)
    ...
    def biasStep(self) -> None:
        ...
        nearestnode = RRT_utils.nearestNeighbour(targetnode,self.startnode,self.randomtree,self.RRTstar)
        # generate step
    ...
    def pathToGoal(self,latestgoalnode:TreeNode) -> bool:
        if not self.goalfound: 
            return False
        # initialise
        RRT_utils.clearTreePath(self.goalpath)
    ...
    def setExclusionZone(self) -> None:
        ...
        for node in self.randomtree.nodes:
            if RRT_utils.nodeDistance(node,self.goalnode) < exclusionradius:
                node.memberofpath = True
    ...
    # smooth final path using Line of Sight algorithm
        ...
        if not self.goalfound: 
            return False
        RRT_utils.clearTreePath(self.goalpathLOS)    #todo, goalpathLOS used anywhere?
        ...
        while LOSbasenode.location is not self.startnode.location:
            ...
            if not RRT_utils.crossCircleObstacle(node,LOSbasenode,self.obstacles,self.SAFETYMARGIN):
